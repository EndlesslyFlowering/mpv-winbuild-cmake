...
download_mpv_package() {
    local package_url="https://codeload.github.com/EndlesslyFlowering/mpv-packaging/zip/master"
    if [ -e mpv-packaging.zip ]; then
        echo "Package exists. Check if it is newer.."
        remote_commit=$(git ls-remote https://github.com/EndlesslyFlowering/mpv-packaging.git master | awk '{print $1;}')
